<template>
  <el-dialog :title="dataForm.shopUserId? $t('table.edit'): $t('table.create')" :visible.sync="visible">
    <el-form ref="dataForm" :rules="dataRule" :model="dataForm" label-position="right" label-width="80px">
      <!-- 昵称 -->
      <el-form-item :label="$t('business.shopUser.nickName')" prop="nickName">
        <el-input v-model="dataForm.nickName" />
      </el-form-item>
      <!-- 头像 -->
      <el-form-item :label="$t('business.shopUser.avatar')" prop="avatar" label-width="80px">
        <img-upload v-model="dataForm.avatar" />
        <!-- <pic-upload v-model="dataForm.avatar" /> -->
      </el-form-item>
      <!-- 员工编号 -->
      <el-form-item :label="$t('business.shopUser.code')" prop="code">
        <el-input v-model="dataForm.code" />
      </el-form-item>
      <!-- 联系方式 -->
      <el-form-item :label="$t('business.shopUser.phoneNum')" prop="phoneNum">
