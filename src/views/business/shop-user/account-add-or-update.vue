<template>
  <el-dialog :title="dataForm.hasAccount? $t('table.edit'): $t('table.create')" :visible.sync="visible">
    <el-form ref="dataForm" :rules="rules" :model="dataForm" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
      <!-- 用户名 -->
      <el-form-item :label="$t('business.shopUserAccount.username')" prop="username">
        <el-input v-model="dataForm.username" />
      </el-form-item>
      <!-- 密码 -->
      <el-form-item :label="$t('business.shopUserAccount.password')" prop="password">
        <el-input v-model="dataForm.password" show-password />
      </el-form-item>
      <!-- 状态 -->
      <el-form-item :label="$t('business.shopUserAccount.status')" prop="status">
...
<script>
import * as api from '@/api/business/shop-user-account'
