<template>
  <div class="app-container mod-hot-search">
    ...
    <div class="filter-container">
      ...
      <el-button v-permission="['business:hotSearch:save']" size="mini" icon="el-icon-plus" type="primary" class="filter-item" @click="addOrUpdateHandle()">{{ $t('table.create') }}</el-button>
    </div>
    ...
    >
      <!-- 热搜标题 -->
      <el-table-column :label="$t('business.hotSearch.title')" prop="title" align="center">
        <template slot-scope="{row}">
          <span>{{ row.title }}</span>
        </template>
      </el-table-column>
      <!-- 内容 -->
      <el-table-column :label="$t('business.hotSearch.content')" prop="content" align="center">
        <template slot-scope="{row}">
          <span>{{ row.content }}</span>
        </template>
      </el-table-column>
      <!-- 创建时间 -->
      <el-table-column :label="$t('business.hotSearch.createDate')" prop="createTime" align="center">
        <template slot-scope="{row}">
          <span>{{ row.createTime }}</span>
        </template>
      </el-table-column>
      <!-- 顺序 -->
      <el-table-column :label="$t('business.hotSearch.seq')" prop="seq" align="center">
        <template slot-scope="{row}">
          <span>{{ row.seq }}</span>
        </template>
      </el-table-column>
      <!-- 状态 0下线 1上线 -->
      <el-table-column :label="$t('business.hotSearch.status')" prop="status" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column :label="$t('table.actions')" align="center" width="230" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button
            v-permission="['business:hotSearch:update']"
            type="primary"
            size="mini"
            icon="el-icon-edit"
          ...
          <el-button
            v-permission="['business:hotSearch:delete']"
            type="danger"
            size="mini"
            icon="el-icon-delete"
...
import * as api from '@/api/business/hot-search'
