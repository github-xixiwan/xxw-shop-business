<template>
  <div class="app-container">
    ...
    >
      <!-- 发布时间 -->
      <el-table-column :label="$t('business.notice.publishTime')" prop="publishTime" align="center">
        <template slot-scope="{row}">
          <span>{{ row.publishTime }}</span>
        </template>
      </el-table-column>
      <!-- 公告标题 -->
      <el-table-column :label="$t('business.notice.title')" prop="title" align="center">
        <template slot-scope="{row}">
          <span>{{ row.title }}</span>
        </template>
      </el-table-column>
      <!-- 状态(1:公布 0:撤回) -->
      <el-table-column :label="$t('business.notice.status')" prop="status" align="center">
        <template slot-scope="{row}">
          <el-tag :type="row.status === 0 ? 'danger' : ''">{{ ['撤销','公布'][row.status] }}</el-tag>
        </template>
      </el-table-column>
      <!-- 是否置顶 -->
      <el-table-column :label="$t('business.notice.isTop')" prop="isTop" align="center">
        <template slot-scope="{row}">
...
import * as api from '@/api/business/notice'
