<template>
  ...
  >
    ...
    <div class="prods-select-body">
      ...
      <!-- 分页条 -->
      <!-- <pagination v-show="pageVO.totalRow>0" :total="pageVO.totalRow" :page.sync="pageQuery.pageNum" :limit.sync="pageQuery.pageSize" @pagination="getBrandList()" /> -->
    </div>
    <span slot="footer">
...
<script>
import { getBrandByCategoryId } from '@/api/goods/brand'
      ...
      this.brandListSelections.forEach(item => {
        ...
        if (brandIndex === -1) {
          brands.push({
            ...
            brandImgUrl: (item.imgUrl).indexOf('http') === -1 ? item.imgUrl : item.imgUrl
          })
        }
      })
